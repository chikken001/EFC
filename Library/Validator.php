...
class Validator
{
	...
	public function is_Code_pays($code)
	{
		if (isset($this->language_codes[$code]))
		{
			return true ;
		}
	...
	{
		$datetime_parse = date_parse_from_format("Y-m-d H:i:s", $date) ;
		$date_parse = date_parse_from_format("Y-m-d", $date) ;
		
		if(!$date instanceof \DateTime && $date_parse['error_count'] != 0 && $datetime_parse['error_count'] != 0)
		{
