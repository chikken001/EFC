...
class Pdo
{
			...
			foreach ($attributs as $nom => $valeur) 
			{
				if($nom  != 'erreurs' && $nom  != 'id' && $nom  != 'validator' && isset($this->types[$nom]))
				{
					if(isset($valeur))
					{
						...
						{
							$date_parse = date_parse_from_format("Y-m-d", $valeur) ;
							$date_parse['error_count'] != 0 ? $valeur = $this->manager->date_format($valeur) : $valeur = $valeur ;
						}
						elseif($this->types[$nom] == 'datetime')
						{
							$date_parse = date_parse_from_format("Y-m-d H:i:s", $valeur) ;
							$date_parse['error_count'] != 0 ? $valeur = $this->manager->datetime_format($valeur) : $valeur = $valeur ;
						}
	...
	{
		$associate = $this->getAssociate($id, $target, $args = array($target => $id_target)) ;
		if(!empty($associate))
		{
