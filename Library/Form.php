...
class Form
{
	...
	public function __construct($entity, array $fields, \Library\Application $app, \Library\Managers $managers)
	{
		...
		{
			if(is_string($field) && $conf[0] != 'div' && $conf[0] != '/div' && $conf[0] != 'p' && $conf[0] != 'h' && $conf[0] != 'label' && $conf[0] != 'legend')
			{
				if($field != 'valide_form')
				{
					if($conf[0] == 'multiform')
					{
						...
						if(isset($conf[3]) && (!is_bool($conf[3]) && ($conf[3] != 1 && $conf[3] == 0)))
						{
							throw new \InvalidArgumentException("le paramètre optionel file doit être un booléen");
						}
						elseif(isset($conf[3]) && $conf[3] == 1)
						{
							...
							else
							{
								foreach($conf[2] as $error => $msg)
								{
									if(is_object($this->objet) && !in_array($field, $this->adds_fields))
									{
										...
										{
											throw new \InvalidArgumentException('L\'erreur specifie a "'.$field.'" ('.$const.') n\'est pas defini pour l\'entite : '.$this->entity);
										}
									}
									else
									{
			...
			elseif($conf[0] == 'div' || $conf[0] == '/div' || $conf[0] == 'p' || $conf[0] == 'h' || $conf[0] == 'label' || $conf[0] == 'legend')
			{
				if($conf[0] == 'div')
				{
					if(isset($conf[1]) && !is_string($conf[1]) && !empty($conf[1]))
				...
				elseif($conf[0] == 'p' || $conf[0] == 'legend')
				{
					$balise = $conf[0] ;
					
					if(!isset($conf[1]) || !is_string($conf[1]) || empty($conf[1]))
					{
						throw new \InvalidArgumentException("le contenu de la balise $balise est invalide");
					}
					
					if(isset($conf[2]) && !is_string($conf[2]) && !empty($conf[2]))
					{
						throw new \InvalidArgumentException("La classe n'est pas valide pour la balise $balise : ".$conf[2]);
					}
					
					if(isset($conf[3]) && !is_string($conf[3]) && !empty($conf[3]))
					{
						throw new \InvalidArgumentException("L'id n\'est pas valide pour la balise $balise : ".$conf[3]);
					}
				}
				elseif($conf[0] == 'label')
				{
					if(!isset($conf[1]) || !is_string($conf[1]) || empty($conf[1]))
					{
						throw new \InvalidArgumentException('le contenu de la balise label est invalide');
					}
						
					if(isset($conf[2]) && !is_string($conf[2]) && !empty($conf[2]))
					{
						throw new \InvalidArgumentException('La classe n\'est pas valide pour la balise label : '.$conf[2]);
					}
					
					if(isset($conf[3]) && !is_string($conf[3]) && !empty($conf[3]))
					{
						throw new \InvalidArgumentException("L'attribut for n'est pas valide pour la balise label : ".$conf[2]);
					}
				}
			}
			else
			{
				throw new \InvalidArgumentException('Le type de champ de "'.$field.'" est invalide');
			}
		}
	...
	public function form ()
	{
		...
		{
			$value ='' ;
			if($field != 'valide_form' && $conf[0] != 'div' && $conf[0] != '/div' && $conf[0] != 'multiform' && $conf[0] != 'h' && $conf[0] != 'p' && $conf[0] != 'label' && $conf[0] != 'legend')
			{
				$errors = array () ;
        ...
        if(is_object($this->objet))
        {
                    ...
                    if(isset($conf[0]) && $conf[0] != 'multiform')
                    {
                        ...
                        elseif($conf[0] == 'file')
                        {
                            if(isset($_FILES[$prefix_field.$field]) && $_FILES[$prefix_field.$field]['size'] != 0)
                            {
                                ...
                                if($error == false)
                                {
                                	$this->objet->$field() ? $nom = $this->objet->$field() : $nom = null ;
                                	$id = $this->objet->id() ;
                                    ...
                                    if(!empty($id) && !empty($nom))
                                    {
                                        ...
                                        {
                                            unlink($dossier.$nom) ? $error = false : $error = true ;
                                        }
                                        else
                                        {
                                            $this->objet->$method('') ;
                                    ...
                                    $nom ? $nom_unique = $nom : $nom_unique = $name.'.'.$extension ;
                                    ...
                                    if($error == true || !move_uploaded_file($_FILES[$prefix_field.$field]['tmp_name'], $dossier.$nom_unique))
                                    {
                                        $add_erreurs[] = $nb_erreur ;
                                    }
                                    elseif($error == false)
                                    {
                                        $this->objet->$method($nom_unique) ;
                                    }
                                }
                            }
                        }
            ...
            if(count($erreurs) === 0 && ($multiform === true || is_array($multiform)))
            {
                ...
                if($validate === false)
                {
                    ...
                    if(empty($entity['id']))
                    {
                    	$id_entity = $this->managers->getManagerOf($this->entity)->dao->lastInsertId() ;
                    	$this->objet->setId($id_entity);
                    }
                    else
                    {
                    	$id_entity = $entity['id'] ;
